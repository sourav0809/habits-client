...
export interface EditGoalsDialogProps {
  ...
  /** When null, dialog is in "create" mode; otherwise "update" with prefill */
  goal: Goal | null;
  onSuccess?: () => void;
}
...
export function EditGoalsDialog({
  ...
  useEffect(() => {
    if (open) {
      setKcalGoal(String(goal?.targetCalories ?? DEFAULT_CALORIE_GOAL));
      setWaterGoal(String(goal?.targetWaterMl ?? DEFAULT_WATER_GOAL_ML));
    }
  }, [open, goal?.targetCalories, goal?.targetWaterMl]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Set your daily calorie and water intake targets. These goals will
